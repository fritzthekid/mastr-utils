...
class StorageImpactAnalyzer:
    ...
    def print_summary(self, results_df):
        ...
        print(f"\nScenario: 2× Wind + 2× Solar (enhanced renewables)")
        print(f"Storage power rating: 0.5 × capacity (e.g., 100 GWh = 50 GW)")
        print(f"Analysis period: {self.enhanced_data.index.min().strftime('%Y-%m-%d')} to {self.enhanced_data.index.max().strftime('%Y-%m-%d')}")
        ...
        print(f"   Without storage:")
        print(f"     Renewable share: {baseline['renewable_share_percent']:.1f}%")
        print(f"     Residual needed: {baseline['residual_share_percent']:.1f}%")
        
        print(f"\n   With {max_storage['capacity_gwh']:.0f} GWh storage:")
        print(f"     Renewable share: {max_storage['renewable_share_percent']:.1f}%")
        print(f"     Residual needed: {max_storage['residual_share_percent']:.1f}%")
        print(f"     Improvement: +{max_storage['renewable_share_percent'] - baseline['renewable_share_percent']:.1f} percentage points")
        
        # Find 90% renewable threshold
        ...
        # Find 95% renewable threshold  
        threshold_95 = results_df[results_df['renewable_share_percent'] >= 95]
        if not threshold_95.empty:
            first_95 = threshold_95.iloc[0]
            print(f"\n   For 95% renewable share:")
            print(f"     Required storage: {first_95['capacity_gwh']:.0f} GWh")
            print(f"     Power rating: {first_95['power_gw']:.0f} GW")
        
        print(f"\n💡 INSIGHTS:")
        print(f"   • Doubling wind/solar increases renewable potential significantly")
        print(f"   • Storage effectiveness has diminishing returns")
        print(f"   • Even large storage cannot eliminate all residual energy needs")
        print(f"   • Battery utilization decreases with larger capacities")
